...
public class AccountNumberScheduler {
    ...
    @Scheduled(cron = "${account.number.generate.debit.cron}")
    public void generateDebit() {
        freeAccountNumbersService.generateFreeAccountNumbers(AccountType.DEBIT, batchSize);
    }

    @Scheduled(cron = "${account.number.generate.credit.cron}")
