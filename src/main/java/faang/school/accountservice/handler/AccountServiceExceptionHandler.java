...
public class AccountServiceExceptionHandler {
    ...
    private ResponseEntity<AccountServiceErrorResponseDto> createErrorResponse(String errorMsg,
                                                                            HttpStatus status,
                                                                            Exception ex) {
        log.error("Error in account-service: {}, response status {}", errorMsg, status, ex);
        AccountServiceErrorResponseDto response =
                new AccountServiceErrorResponseDto(errorMsg, LocalDateTime.now(), status.value());
